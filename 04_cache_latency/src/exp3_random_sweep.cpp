...
// change is build_sequential_chain() -> build_random_chain(), i.e. which
// order the same N cache lines are visited in.
//
// Hypothesis (written *before* measuring): a hardware prefetcher can only
// exploit a predictable address stream. build_random_chain()'s single
...
int main() {
    std::cout << "=== Experiment 3: random sweep (modified) ===\n\n";
